...
MAX_QUERIES_TO_TEST  = 200

INDEX_DIR  = "/kaggle/input/datasets/areenvaghasiya/offline-index-areen1/offline_index"
...
# ==========================================
# 2. FIND DATASET PATHS DYNAMICALLY
# ==========================================
...
PARTITION_FILE = next((p for p in possible_partition_paths if os.path.exists(p)), None)
...
def normalize_id(item_id):
    # Normalize item ID so id_0001 == 0001
...
def process_query_and_search(query_path, top_k=15):
    ...
    # Fetch 3x candidates to leave room for re-ranking
    pool_size  = top_k * 3
